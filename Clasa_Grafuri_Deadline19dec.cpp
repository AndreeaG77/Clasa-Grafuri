...
public:
    Graf();
    Graf(int nr_noduri, int nr_muchii, const int* m, const int* cst);
    ~Graf();
    void BFS();
    void DFS();
    void dfs(int nod, int viz[nmax], vector<int> la[nmax]);
    void HHakimi(int n, vector<int> v);
    void CTC();
    ...
    void Flux_Max();
    void FloydWarshall(int n, int m[nmax][nmax]);
    void Dm_Arb();
    int* bfs_arb(vector<int> la[nmax], int s);
...
Graf::Graf(int nr_noduri, int nr_muchii, const int* m, const int* cst){
...
void Graf::HHakimi(int n, vector<int> v){
    int i,x,ok=0;
...
void Graf::FloydWarshall(int n, int m[nmax][nmax]){
    int i,j,x,k;
    for(i=1;i<=n;i++){
        for(j=1;j<=n;j++){
            if(m[i][j]==0 && i!=j)
...
int* Graf::bfs_arb(vector<int> la[nmax], int s){
    queue <int> q;
...
void Graf::Dm_Arb(){
    ...
    int *sol_aux = bfs_arb(la,1);
    int *sol = bfs_arb(la,sol_aux[0]);
    cout<<sol[1];
}
